...
\begin{exemple}[Contre-exemple à la convergence monotone pour une suite non croissante]
	$$f_n = \1_{[n, \infty[}$$
	$$f_n \to 0, \ \forall x$$
...
\begin{proof}
	Nous allons utiliser ce schéma de preuve:
	\begin{enumerate}
		\item Vrai pour $\1_A$ ?
		\item Vrai pour $g$ étagée positive ?
		\item Vrai pour  $g$ mesurable positive ?
	\end{enumerate}
	...
	\begin{enumerate}
		...
		\item Si $g = \sum_{i=1}^n \alpha_i \1_{A_i}$, alors
		      \begin{eqnarray*}
			      ...
			      &=& \int g f d \mu
		      \end{eqnarray*}
		\item Si $g$ est mesurable positive, alors On prend $g_n \uparrow g$ une suite de fonctions étagées positives.
...
\begin{exemple}[loi normale sur $\R$]
...
\begin{exemple} $([0,1], \bor([0,1]),\lambda)$\\
	\begin{eqnarray*}
		f_{2k} &=& \1_{[\frac{1}{2}, 1]}\\
